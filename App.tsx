...
const EmptyState: React.FC<{ onAction: () => void }> = ({ onAction }) => (
  ...
  >
    ...
    <button onClick={onAction} className="text-blue-600 font-semibold hover:text-blue-700 underline">Check sample text</button>
  </motion.div>
);
...
const App: React.FC = () => {
  ...
  const downloadPPTX = () => {
    alert("In a production environment, this would trigger PptxGenJS to create a .pptx file with the chosen theme.");
  };
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
            ...
            <AnimatePresence>
              {data && (
                <motion.button 
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full font-semibold transition-all shadow-md active:scale-95"
                >
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
            <label className="text-sm font-bold text-slate-700 mb-3 uppercase tracking-wider">Source Content</label>
            <textarea 
              value={inputText}
          ...
          <AnimatePresence>
            {data && (
              ...
              >
                <label className="text-sm font-bold text-slate-700 mb-4 uppercase tracking-wider block">Visual Theme</label>
                <div className="grid grid-cols-2 gap-3">
                  {(Object.keys(THEMES) as ThemeType[]).map((t) => (
                    <button
                      key={t}
                      onClick={() => setCurrentTheme(t)}
                      className={`p-3 rounded-xl border-2 transition-all text-left capitalize font-semibold ${
                        currentTheme === t 
            ...
            {isGenerating ? (
              ...
              >
                <div className="flex items-center justify-between">
                  <h2 className="text-2xl font-bold text-slate-900 truncate max-w-md">{data.title}</h2>
                  <div className="flex items-center gap-2">
                    <button 
                      onClick={() => setActiveSlideIndex(Math.max(0, activeSlideIndex - 1))}
                      disabled={activeSlideIndex === 0}
                      className="p-2 rounded-full hover:bg-white border border-transparent hover:border-slate-200 disabled:opacity-20 transition-all active:scale-90"
                    >
                    ...
                    >
                      <div className={`w-full h-full p-2 text-[6px] text-left leading-[1] ${THEMES[currentTheme].bg} ${THEMES[currentTheme].text}`}>
                        ...
                        {slide.generatedImageUrl && (
                          <div className="absolute bottom-1 right-1 w-2 h-2 bg-emerald-500 rounded-full shadow-sm" />
                        )}
                      </div>
                    </button>
                  ))}
                ...
                >
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Presenter Notes</h4>
                  </div>
      ...
      <footer className="bg-white border-t border-slate-200 py-6 mt-12">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-4 text-slate-500 text-sm">
          <p>© 2024 SlideCraft AI. Powered by Gemini Pro & Imagen.</p>
